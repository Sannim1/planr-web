...
##################### Initialization Operations ##########################
    ...
    #   description : Constructor of the GA
    ...
    #   description : Gets VSTS work items to create feature items for the algorithms,
    ...
    #   description : Creates a map that shows the coupled features
    ...
    #   description : Creates a map that shows the dependent features of a feature
    ...
    #   description : Creates the list of initial feasible features
    def initialize_feasible_features(self):
        ...
        for index,feature in self.features.items():
            ...
            if feature[4] != None:
                continue
            # if preceded_by == None and it's not coupled with other features then it's feasible
            if index not in self.coupling_map:
                self.initial_feasible_features.append(index)
                continue
            # if it's not the first feature in its coupling group then we have already covered it
    ...
    #   description : Specifies the containers of individual and population, registers evolutionary tools 
    ...
    #   description: Pops the first feature from a randomly generated release plan
    ...
    #   description : Randomly generates a valid release plan
    def get_initial_release_plan(self):
        ...
        for release_number in xrange(1, self.num_releases + 1):
            while True:
                ...
                # if the effort of the feature(or features) is greater than the remaining effort of the release
                # then break and move to the next release
...
##################### GA Operations ##########################
    ...
    #   description : Evaluates how fit a release plan is
    ...
    def evalReleasePlan(self, individual):
        ...
        for feature, release in enumerate(individual):
            ...
            if self.features[feature][4] != None and self.features[feature][4] != self.features[feature][3]:
                ...
                # if the implementation of the preceded feature(parent) is after the dependent(child)
                # then the release plan is invalid
            ...
            if self.features[feature][5] != None and self.features[feature][5] != self.features[feature][3]:
                ...
                # if the implementations of the coupled features are not the same
                # then the release plan is invalid
                if release != couplingRelease:
                    return sys.maxint, 0
            # if feature is implemented, update benefit, effort and penalty of the release
            if release !=0:
            ...
            # if the sum effort of the features implemented in a release is greater than team capacity
            # then the release plan is invalid
    ...
    #   description : Crossovers release plans to generate 4 offsprings that share their characteristics 
    ...
    #   description : Mutates a release plan to introduce variance in solutions 
    ...
    #   description: Implementation of the core of the Genetic Algorithm
    ...
    def custom_algorithm(self, pop, toolbox, mu, CXPB, MUTPB, NGEN, halloffame):
        ...
        # Register best individuals in halloffame
        ...
        for g in range(NGEN):
            ...
            # The population is replaced by selecting the fittest individual from previous population and offsprings
            pop[:] = toolbox.select(selected_population + offspring, mu)

            # Update halloffame with best offsprings
            halloffame.update(pop)


##################### Run-time Operation ##########################

    #   description : This method runs the GA to return sub-optimal release plans
    def generate(self):
...
##################### Supporting Operations ##########################    
    ...
    #   description : Returns the combined effort of all the coupled feature to this feature
    ...
    #   description : Returns the list of coupled feature to this feature
    #       feature_index : the index of the featuere in the algorithm's feature item list
    ...
    #   description : Returns the list of features that are dependent to this feature
    #       feature_index : the index of the featuere in the algorithm's feature item list
    ...
    #   description : Checks if feature is feasible based on its constraints
    #       feature_index: the index of the featuere in the algorithm's feature item list
    ...
    #   description : Checks if team capacity is greater equal to the sum effort of all features
    ...
    #   description : Checks if features are coupled
    ...
    #   description : Transforms single release plan into list of releases containing ids of the features 
